...
class TestCET6Lesson:
    ...
    def cards(self, e01, cet6_spec):
        return run_all(LessonRuntime.build(e01, cet6_spec, Progress()))
...
class TestNoWordLesson:
    ...
    def test_能跑完(self, e01, no_word_spec):
        rt = LessonRuntime.build(e01, no_word_spec, Progress())
        cards = run_all(rt)
        assert rt.finished
        assert len(cards) > 10
...
class TestMastery:
    def test_课内不产生掌握(self, e01, cet6_spec):
        p = Progress()
        run_all(LessonRuntime.build(e01, cet6_spec, p), correct=True)
        for cid in cet6_spec.chunk_ids:
    ...
    def test_答错清零(self, e01, cet6_spec):
        p = Progress()
        run_all(LessonRuntime.build(e01, cet6_spec, p), correct=False)
        e = p.entry("chunks", "muddy_puddles")
...
class TestResume:
    def test_续上(self, e01, cet6_spec):
        ...
        rt2 = LessonRuntime.restore(e01, cet6_spec, p, rt.to_dict())
    ...
    def test_有复习和抽检时卡序仍可复现(self, e01, cet6_spec):
        ...
        rt = LessonRuntime.build(e01, cet6_spec, p, known=known)
        before = [c.card_id for c in rt.cards]
        for _ in range(7):
            c = rt.current()
            rt.answer(True) if c.needs_answer else rt.advance()

        rt2 = LessonRuntime.restore(e01, cet6_spec, p, rt.to_dict(), known=known)
    ...
    def test_快照含复习与抽检选择(self, e01, cet6_spec):
        p = Progress()
        p.record("words", "george", "a2i", True)
        rt = LessonRuntime.build(e01, cet6_spec, p, known={"words": ["daddy"]})
        snap = rt.to_dict()
...
class TestBonus:
    def test_顺带条目各1题(self, e01):
        ...
        cards = run_all(LessonRuntime.build(e01, spec, Progress()))
